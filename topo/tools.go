...
func (buffer Buffer) ComputeParameters() {
...
func (buffer Buffer) GetAltitude(loc image.Point) int16 {
	return buffer.Data[loc.Y*buffer.Width+loc.X]
}

// SetAltitude set an altitude in the buffer
func (buffer Buffer) SetAltitude(loc image.Point, alt int16) {
	buffer.Data[loc.Y*buffer.Width+loc.X] = alt
}

//Extract extract a rectangle in the buffer
func (buffer Buffer) Extract(start image.Point, end image.Point) (Buffer, error) {
